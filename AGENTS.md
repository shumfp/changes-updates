...
A shared internal tool that captures registration changes from WhatsApp/email, structures them, syncs updates to Google Sheets with colour-coded highlights, and auto-acknowledges requesters.
...
- **Commit as your GitHub identity, or Vercel will block the deploy.** Vercel verifies that
  ...
  ```
  git config user.email "296501517+shumfp@users.noreply.github.com"
  git config user.name "shumfp"
  ```
